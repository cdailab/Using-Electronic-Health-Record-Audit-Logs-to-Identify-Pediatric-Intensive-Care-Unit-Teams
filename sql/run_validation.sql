...
-- Links REDCap team observation names to EHR user IDs. Includes unit column (PICU/NICU/CVICU).
-- ======================================================================================
CREATE OR REPLACE TABLE `INSERT_TABLE_NAME.val_redcap_enriched` AS
WITH base AS (
  SELECT
    rt.csn,
    rt.unit,
  ...
  FROM `INSERT_TABLE_NAME.val_redcap_teams` AS rt
...
GROUP BY bw.csn, bw.date, bw.unit
HAVING
...
-- ======================================================================================
CREATE OR REPLACE TABLE `INSERT_TABLE_NAME.val_audit_logs` AS
WITH RedcapCSNDates AS (
  SELECT DISTINCT csn, access_date
  FROM `INSERT_TABLE_NAME.val_redcap_enriched`
)
SELECT DISTINCT
  ual.process_id,
  ual.access_instant,
...
-- ======================================================================================
CREATE OR REPLACE TABLE `INSERT_TABLE_NAME.val_note_audit_logs` AS
...
SELECT
  ...
  d.string_value  AS dtl_string_value
FROM `INSERT_CLARITY_ACCESS_LOG_TABLE_NAME` AS ual
JOIN `INSERT_TABLE_NAME.val_redcap_enriched` AS pdcsn
...
-- ======================================================================================
CREATE OR REPLACE TABLE `INSERT_TABLE_NAME.val_note_signers` AS
WITH FrontlineNoteActions AS (
  ...
  FROM `INSERT_TABLE_NAME.val_note_audit_logs` nu
  WHERE
...
-- ======================================================================================
CREATE OR REPLACE TABLE `INSERT_TABLE_NAME.val_activity_features` AS
WITH PatientProviderDayMetrics AS (
  ...
  SELECT
    ...
    COUNT(*)                                                                                         AS n_total
  FROM `INSERT_TABLE_NAME.val_audit_logs` nu
...
HourlyActionCounts AS (
  ...
  FROM `INSERT_TABLE_NAME.val_audit_logs`
